 ...
 private:
  ...
  class CallData {
   public:
    ...
    // with the gRPC runtime.
    CallData(ImageMirror::AsyncService* service, ServerCompletionQueue* cq)
    ...
    void Proceed() {
      if (status_ == CREATE) {
        ...
        // start processing FlipImage requests. In this request, "this" acts are
        // the tag uniquely identifying the request (so that different CallData
        // instances can serve different requests concurrently), in this case
        // the memory address of this CallData instance.
        service_->RequestFlipImage(&ctx_, &request_, &responder_, cq_, cq_,
                                  this);
    ...
    void Process(){
      FlipType fliptype = request_.flip_type();

      if(fliptype == FlipType::BOTH){
        // The actual processing.
        std::string flipped (request_.image());
        std::reverse(flipped.begin(), flipped.end());
        reply_.set_image(flipped);
      }
      else if(fliptype == FlipType::HORIZONTAL){
        reply_.set_image(ImageOperations::FlipHorizontal(request_.image().c_str(), request_.width(), request_.height()));
      }
      else if(fliptype == FlipType::VERTICAL){
        reply_.set_image(ImageOperations::FlipVertical(request_.image().c_str(), request_.width(), request_.height()));
      }
   ...
   private:
    ...
    // server.
    ImageMirror::AsyncService* service_;
    ...
    ImageFlipRequest request_;
    // What we send back to the client.
    ImageFlipReply reply_;

    // The means to get back to the client.
    ServerAsyncResponseWriter<ImageFlipReply> responder_;
